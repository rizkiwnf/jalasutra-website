...
const Navbar = () => {
    ...
    return (<
        ...
        ul className="flex flex-col md:flex-row gap-3 md:gap-10 font-bold" > {
                            Links.map((links) => (<
                                li > < Link to={links.link}
                                    id={links.id} > {links.name} < /Link></li >
                            ))
                        } <
        /ul> <
        /div> <
        ...
        div onClick={
                                        () => setOpen(!open)}
                                    className="icon-menu md:hidden px-3" > {open ? < FaBarsStaggered size={25} /> : <FaXmark size={25} />} <
        /div> <
        /div> { /* resp */} <
